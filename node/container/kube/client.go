...
func (c *client) Deploy(ctx context.Context, deployment builder.IClusterDeployment) error {
	...
	for svcIdx := range group.Services {
		...
		c.providerConfig.IngressHostName = strings.Trim(strings.Trim(c.providerConfig.IngressHostName, "http://"), "https://")
		...
		for _, expose := range service.Expose {
			if builder.ShouldBeIngress(expose) {
				...
				if c.providerConfig.IngressCertificatePath != "" && c.providerConfig.IngressCertificateKeyPath != "" {
					secret, err = createTLSSecretByCustom(ctx, c.kc, ns.Name(), c.providerConfig.IngressHostName, c.providerConfig.IngressCertificatePath, c.providerConfig.IngressCertificateKeyPath)
					if err != nil {
