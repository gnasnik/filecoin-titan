...
func (m *SQLDB) AddNewProvider(ctx context.Context, provider *types.Provider) error {
	qry := `INSERT INTO providers (id, owner, remote_addr, ip, state, created_at, updated_at) 
		        VALUES (:id, :owner, :remote_addr, :ip, :state, :created_at, :updated_at) ON DUPLICATE KEY UPDATE  owner=:owner, remote_addr=:remote_addr, 
